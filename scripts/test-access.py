...
# shares، گیت‌وی، پاسخ گیت‌وی به هر مبدأ، نتیجه آماده پینگ شاهد، انتظار، توضیح
ROUTING_CASES = [
    ...
    (True,  "1.2.3.1", {"9.9.9.9": False, "1.2.3.5": True}, None, False,
     "هم‌ساب‌نت ولی گیت‌وی جواب نمی‌دهد — روت نشده، نه اثبات"),
    (True,  "1.2.3.1", {"9.9.9.9": True}, None, True, "هم‌ساب‌نت و گیت‌وی جواب داد"),
    (False, None,      {}, None, None,  "رنج متفاوت، گیت‌وی ثبت نشده"),
    (False, "1.2.3.1", {"9.9.9.9": True}, None, True, "رنج متفاوت، گیت‌وی جواب داد"),
    # مهم‌ترین حالت: گیت‌وی ساکت است. بدون پینگ شاهد، این False می‌شد و
    # ادمین دنبال مشکلی می‌رفت که وجود ندارد.
    (False, "1.2.3.1", {"9.9.9.9": False, "1.2.3.5": False}, None, None,
     "گیت‌وی به هیچ‌کس جواب نمی‌دهد — بی‌نتیجه، نه منفی"),
    (False, "1.2.3.1", {"9.9.9.9": False, "1.2.3.5": True}, None, False,
     "گیت‌وی به آدرس اصلی جواب می‌دهد ولی به این نه — واقعاً روت نشده"),
    # نتیجه پینگ شاهد یک بار برای کل دور حساب می‌شود و آماده داده می‌شود؛
    # با صدها آدرس، محاسبه‌اش به‌ازای هر آدرس دور را تمام‌نشدنی می‌کرد
    (True,  "1.2.3.1", {"9.9.9.9": False}, False, None,
     "شاهد آماده می‌گوید گیت‌وی ساکت است — بی‌نتیجه"),
    (True,  "1.2.3.1", {"9.9.9.9": False}, True, False,
     "شاهد آماده می‌گوید گیت‌وی زنده است — روت نشده"),
]
...
def check_routing(agent):
    ...
    try:
        for shares, gw, table, alive, expected, name in ROUTING_CASES:
            answers = table
            got = agent.routing_test("9.9.9.9", gw, shares, "1.2.3.5", alive)
            if got == expected:
