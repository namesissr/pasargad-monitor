...
۴. **offset پردازش پیام‌ها ذخیره می‌شود، همیشه جلو می‌رود، و از
   دیتابیس خوانده می‌شود نه از کش تنظیمات.** بدون ذخیره، ورکر پس از هر
   ری‌استارت همان /start قدیمی را دوباره پردازش می‌کند. اگر فقط هنگام
   موفقیت جلو می‌رفت، یک پیام خراب تا ابد تکرار می‌شد. و اگر از
   settings() خوانده شود — که پانزده ثانیه کش دارد در حالی که این چرخه
   هر ده ثانیه است — هر پیام دو بار پردازش می‌شود.
...
def main():
    ...
    source_checks = [
        ...
        (mig, "مهاجرت ۰۴۳", "کلید offset در تنظیمات".replace("کلید offset در تنظیمات",
         "telegram_updates_offset"), "کلید offset در تنظیمات"),
        (link, "telegram-link",
         "SELECT value FROM settings WHERE key = 'telegram_updates_offset'",
         "offset مستقیم از دیتابیس خوانده می‌شود، نه از کش"),
    ...
    # offset هرگز نباید از شیء settings() خوانده شود.
    #
    # settings() پانزده ثانیه کش دارد و این چرخه هر ده ثانیه اجرا
    # می‌شود. با کش، offsetی که دور قبل نوشته شده هنوز قدیمی دیده
    # می‌شود و همان به‌روزرسانی‌ها دوباره پردازش می‌شوند — برای کد
    # اتصال یعنی پیام «کد معتبر نیست» درست بعد از اتصال موفق.
    if re.search(r"\bs\.telegram_updates_offset\b", link):
        failures += 1
        print("شکست  کد واقعی (telegram-link): offset از کش تنظیمات خوانده می‌شود")
    else:
        print("گذشت  کد واقعی (telegram-link): offset از کش تنظیمات خوانده نمی‌شود")
