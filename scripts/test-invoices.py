...
def main():
    ...
    source_checks = [
        ...
        (ret, "return", "payment_ref = COALESCE($4, payment_ref)",
         "شناسه پرداخت هنگام شکست هم ذخیره می‌شود"),

        # ── کد پرداخت در تأیید ────────────────────────────────
        #
        # درگاه با پیام «مقدار فیلد 'کد پرداخت' اجباری است» تأیید را رد
        # می‌کرد چون فقط refId و مبلغ می‌رفت. پول کم می‌شد و فاکتور باز
        # می‌ماند.
        (pp, "payping", "paymentCode: code, code }",
         "کد پرداخت در تأیید نسخه ۲ هم فرستاده می‌شود"),

        # ── تأیید دوباره بدون پرداخت مجدد ────────────────────
        (admin, "admin", "retry_verify",
         "فاکتور ناموفق بدون پرداخت دوباره قابل تأیید است"),
        (admin, "admin", "JSON.parse(inv.callback_raw)",
         "شناسه از پارامترهای ذخیره‌شده هم خوانده می‌شود"),
    ]
